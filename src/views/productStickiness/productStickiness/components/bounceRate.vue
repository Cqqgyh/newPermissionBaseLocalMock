<template>
  <el-card class="m-t-20">
    <template #header>
      <div class="font-20-bold">跳出率</div>
    </template>
    <!--      时间选择-->
    <div>
      <span>时间：</span>
...
import { BounceRateInterfaceRes } from '@/api/productStickiness/types'
import { getBounceRate } from '@/api/productStickiness'
//#region <表单相关>
// 搜索参数
...
const seriesHourlyList = computed(() => {
  return pageData.value.map((item) => item.hourlyBounceRate)
})
const seriesDailyList = computed(() => {
  return pageData.value.map((item) => item.dailyBounceRate)
})
// 数据总览数据
const pageData = ref([] as BounceRateInterfaceRes[])
// 展示数据图表配置
const option = ref({
  title: {
    text: '指标',
...
async function getBounceRateHandle() {
  try {
    const res = await getBounceRate(searchParams.value)
