<template>
  <el-card class="m-t-20">
    <template #header>
      <div class="font-20-bold">人均PV</div>
    </template>
    <!--      时间选择-->
    <div>
      <span>时间：</span>
...
import { getPagesPerVisit } from '@/api/productStickiness'
import { PagesPerVisitInterfaceRes } from '@/api/productStickiness/types'
...
const seriesHourlyList = computed(() => {
  return pageData.value.map((item) => item.hourlyPagesPerVisit)
})
const seriesDailyList = computed(() => {
  return pageData.value.map((item) => item.dailyPagesPerVisit)
})
// 数据总览数据
const pageData = ref([] as PagesPerVisitInterfaceRes[])
// 展示数据图表配置
const option = ref({
  title: {
    text: '指标',
...
watch(
  searchParams,
  () => {
    getPagesPerVisitHandle()
  },
  { deep: true },
)
async function getPagesPerVisitHandle() {
  try {
    const res = await getPagesPerVisit(searchParams.value)
